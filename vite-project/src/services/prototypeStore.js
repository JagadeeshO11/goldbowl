...
export function registerDeliveryPartner(profile) { const partner = { id: crypto.randomUUID(), role: 'delivery', verificationStatus: 'PENDING', fee: 700, feeStatus: 'PENDING', createdAt: new Date().toISOString(), ...profile }; state = { ...state, deliveryPartners: [partner, ...state.deliveryPartners] }; addNotification('admin', 'Delivery onboarding submitted', `${partner.name} submitted a partner application.`); persist(); return partner }
