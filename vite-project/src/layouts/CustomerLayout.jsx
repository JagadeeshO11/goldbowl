    ...
    <div className="mobile-status-bar">
      ...
      <div className="status-bar-right">
        ...
        <div className="battery-indicator">
          <BatteryCharging size={13} className="charging-icon" />
          <span>85%</span>
        </div>
      </div>
    </div>
...
export function CustomerHeader({ onProfile }) {
  ...
  const [selectedBranch, setSelectedBranch] = useState(() => {
    try {
      const stored = sessionStorage.getItem('bowlCustomerBranch')
      return stored ? JSON.parse(stored) : mockBranches[0]
    } catch {
  ...
  const handleSelectBranch = (b) => {
    setSelectedBranch(b)
    sessionStorage.setItem('bowlCustomerBranch', JSON.stringify(b))
    setOpen(false)
  }
        ...
        <header className="goldbowl-customer-header">
          ...
          <div className="goldbowl-header-actions">
            ...
            >
              ...
              <span>{selectedBranch?.name?.replace(' Bowl', '') || 'Select Branch'}</span>
              <ChevronDown size={15}/>
            </button>
        ...
        <div className="branch-modal-overlay" onClick={() => setOpen(false)}>
          <div className="branch-modal" onClick={e => e.stopPropagation()}>
            <div className="branch-modal-head">
              <div>
                <span className="eyebrow">STORE SELECTOR</span>
                <h3>Select Branch</h3>
              </div>
              <button type="button" className="close-btn" onClick={() => setOpen(false)}><X size={18}/></button>
            </div>
              ...
              {mockBranches.map(b => (
                ...
                >
                  ...
                  <div className="branch-meta">
                    ...
                    <span>{b.area} • {b.distance} • Open now</span>
                  </div>
                  {selectedBranch.id === b.id && <Check size={18} className="check-icon"/>}
                </button>
              ))}
            </div>
          </div>
