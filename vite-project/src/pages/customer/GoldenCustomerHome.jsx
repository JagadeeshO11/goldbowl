...
function FoodCard({ product, onAdd }) {
  return <article className="customer-food-card">
    <Link to={`/customer/product/${product.id}`} className="customer-food-img"><img src={product.image} alt={product.name} loading="lazy" /><span className="food-heart" aria-label={`Favourite ${product.name}`}><Heart size={17} /></span></Link>
    <div className="customer-food-info">
      ...
      <h3>{product.name}</h3><p className="food-description">{product.description}</p>
      ...
      <div className="customer-food-bottom"><div><span className="price-label">Price</span><strong>₹{product.price}</strong></div><button className="customer-add" type="button" onClick={() => onAdd(product)}>Add <Plus size={14} strokeWidth={2.8} /></button></div>
    </div>
  </article>
}
...
export function GoldenCustomerHome() {
  ...
  const addToCart = product => { const key = 'goldbowl_cart'; let cart = []; try { cart = JSON.parse(localStorage.getItem(key)) || [] } catch {}; const existing = cart.find(item => item.productId === product.id); const next = existing ? cart.map(item => item.productId === product.id ? { ...item, quantity: item.quantity + 1 } : item) : [...cart, { productId: product.id, quantity: 1 }]; localStorage.setItem(key, JSON.stringify(next)); navigate('/customer/cart') }
  ...
  return <>
    ...
    <div className="customer-home-body">
      <div className="customer-search"><Search size={24} strokeWidth={2} /><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search for your favourite food..." aria-label="Search food" /><Mic size={22} strokeWidth={2} /></div>
      <div className="customer-categories" aria-label="Food categories">{visibleCategories.map(category => <Link key={category.id} to={category.id === 'all' ? '/customer/home' : `/customer/search?category=${category.id}`} onClick={() => setSelected(category.id)} className={`customer-category ${selected === category.id ? 'active' : ''}`}><span className="category-icon">{category.icon || categoryVisuals[category.id] || '🍽️'}</span><span className="category-name">{category.name === 'Signature Bowls' ? 'Bowls' : category.name === 'Fresh & Healthy' ? 'Salads' : category.name}</span></Link>)}</div>
      <section className="customer-hero" aria-label="Featured food offers"><img key={heroProduct.id} src={heroProduct.image} alt={heroProduct.name} /><div className="customer-hero-copy"><span className="hero-eyebrow">{currentHero.eyebrow}</span><h1>{currentHero.title}</h1><p>{currentHero.copy}</p><Link to={`/customer/product/${heroProduct.id}`}>Order Now <ChevronRight size={16} /></Link></div><button className="hero-arrow hero-arrow-left" onClick={previousSlide} aria-label="Previous slide"><ChevronLeft size={18} /></button><button className="hero-arrow hero-arrow-right" onClick={nextSlide} aria-label="Next slide"><ChevronRight size={18} /></button><div className="hero-dots">{heroSlides.map((item, index) => <button key={item.productId} className={index === slide ? 'active' : ''} onClick={() => setSlide(index)} aria-label={`Go to slide ${index + 1}`} />)}</div></section>

      <div className="customer-section-title"><div><span className="section-kicker">HANDPICKED FOR YOU</span><h2>Popular Items</h2></div><Link to="/customer/search">View All <ChevronRight size={18} /></Link></div>
      <div className="customer-popular">{popularProducts.map(product => <FoodCard key={product.id} product={product} onAdd={addToCart} />)}</div>

      {moreProducts.length > 0 && <section className="customer-more-section"><div className="customer-section-title customer-more-title"><div><span className="section-kicker">MORE FROM GOLDEN BOWL</span><h2>Explore More</h2></div><Link to="/customer/search">See Menu <ChevronRight size={18} /></Link></div><div className="customer-more-grid">{moreProducts.map(product => <FoodCard key={product.id} product={product} onAdd={addToCart} />)}</div></section>}
