...
function FoodCard({ product, onAdd }) {
  ...
  return <article className="customer-food-card customer-reference-card">
    <Link to={`/customer/product/${product.id}`} className="customer-food-img">
      ...
      <span className="food-heart"><Heart size={17} /></span>
    </Link>
    <button className="customer-reference-add" type="button" onClick={() => onAdd(product)}>ADD</button>
    <div className="customer-food-info">
      <div className="reference-price-row"><strong>₹{product.price}</strong><del>₹{meta.originalPrice}</del></div>
      <div className="reference-discount">₹{meta.discount} OFF</div>
      <h3>{product.name}</h3>
      <div className="reference-portion">{product.portion}</div>
      <span className="reference-badge">{meta.badge}</span>
    </div>
  </article>
}
...
function ExploreCard({ product, onAdd }) {
  ...
  return <article className="explore-product-card customer-reference-card">
    <Link to={`/customer/product/${product.id}`} className="explore-product-image customer-food-img">
      <img src={product.image} alt={product.name} loading="lazy" />
      <span className="explore-favourite"><Heart size={16}/></span>
    </Link>
    <button className="customer-reference-add" type="button" onClick={() => onAdd(product)}>ADD</button>
    <div className="explore-product-body customer-food-info">
      <div className="reference-price-row"><strong>₹{product.price}</strong><del>₹{meta.originalPrice}</del></div>
      <div className="reference-discount">₹{meta.discount} OFF</div>
      <h3>{product.name}</h3>
      <div className="reference-portion">{product.portion}</div>
