...
export function GoldenCustomerHome() {
  const navigate = useNavigate(); const { products, categories } = usePrototypeContext(); const [selected, setSelected] = React.useState('all'); const [query, setQuery] = React.useState(''); const [slide, setSlide] = React.useState(0); const [vegOnly, setVegOnly] = React.useState(false)
  ...
  const visibleCategories = [{ id: 'all', name: 'All', icon: '🍲' }, ...categories]; const currentHero = heroSlides[slide]; const heroProduct = products.find(product => Number(product.id) === currentHero.productId) || products[0]
  ...
  const filteredProducts = products.filter(product => { const matchesCategory = selected === 'all' || product.category === selected; const matchesSearch = !query.trim() || `${product.name} ${product.description}`.toLowerCase().includes(query.toLowerCase()); const matchesVeg = !vegOnly || !!product.veg; return matchesCategory && matchesSearch && matchesVeg })
  ...
  return <div className="customer-home-body">
    ...
    <section className="customer-hero"><img key={heroProduct?.id} src={heroProduct?.image} alt={heroProduct?.name || 'Golden Food Bowl'} /><div className="customer-hero-copy"><span className="hero-eyebrow">{currentHero.eyebrow}</span><h1>{currentHero.title}</h1><p>{currentHero.copy}</p>{heroProduct && <Link to={`/customer/product/${heroProduct.id}`}>Order Now <ChevronRight size={16}/></Link>}</div><button className="hero-arrow hero-arrow-left" onClick={previousSlide}><ChevronLeft size={18}/></button><button className="hero-arrow hero-arrow-right" onClick={nextSlide}><ChevronRight size={18}/></button><div className="hero-dots">{heroSlides.map((item,index)=><button key={item.productId} className={index===slide?'active':''} onClick={()=>setSlide(index)} aria-label={`Slide ${index+1}`}/>)}</div></section>
