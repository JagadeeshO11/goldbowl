  ...
  return (
    <>
      <div className="admin-stats">
        <div>
          <BarChart3 />
          <strong>₹{sales.toLocaleString('en-IN')}</strong>
        ...
        <div>
          ...
          <span>Live Orders Today</span>
          <small style={{ color: '#ca8a04', fontSize: 9.5, fontWeight: 700 }}>18 currently delivering</small>
        </div>
        <div>
          <Users />
          <strong>1,284</strong>
          <span>Active Customers</span>
          <small style={{ color: '#16a34a', fontSize: 9.5, fontWeight: 700 }}>+42 new today</small>
        </div>
        <div>
          <Truck />
          <strong>32</strong>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
            ...
            <input
              ...
              style={{ paddingLeft: 28, height: 32, borderRadius: 8, border: '1px solid #e2d8c8', fontSize: 11 }}
            />
          </div>
        </div>
      </div>
              ...
              <tr key={o.id}>
                ...
                <td>
                  {o.status !== 'DELIVERED' ? (
                    ...
                    >
                      {o.status === 'READY_FOR_PICKUP' ? 'Assign Delivery →' : 'Advance Status →'}
                    </button>
                  ) : (
      ...
      <div className="table-heading">
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <input
            ...
            style={{ height: 32, width: 80, padding: '0 8px', borderRadius: 8, border: '1px solid #e2d8c8', fontSize: 11 }}
          />
...
function Categories() {
  ...
  const add = () => {
    if (name.trim()) addCategory({ name: name.trim(), icon: '🍲' })
    setName('')
  }
  ...
  return (
    <div className="admin-table-card">
      <div className="table-heading">
        ...
        <span style={{ fontSize: 11, color: '#78716c' }}>{users.length} Registered Accounts</span>
      </div>
      <div style={{ overflowX: 'auto' }}>
        <table>
          <thead>
          ...
          <tbody>
            {users.length ? (
              ...
              <tr>
                <td colSpan="5" style={{ textAlign: 'center', padding: 20, color: '#78716c' }}>
                  No customer signups recorded yet. Use the customer signup flow to create a demo account.
                </td>
              </tr>
            )}
          </tbody>
...
function Delivery() {
  ...
  return (
    <div className="admin-table-card">
      <div className="table-heading">
        <h2>Delivery Partner Verification</h2>
            ...
            {deliveryPartners.length ? (
              deliveryPartners.map(p => (
                <tr key={p.id}>
                  ...
                  <td>{p.mobile}</td>
                  <td>{p.vehicle}</td>
                  <td>{p.documentsVerified ? '✓ Verified' : 'Pending'}</td>
                  <td>₹{p.fee} • {p.feeStatus}</td>
                  <td>
                    <button
                      className="admin-action-btn"
  ...
  return (
    <div className="admin-table-card">
      <div className="table-heading">
        ...
        <span style={{ fontSize: 11, color: '#78716c' }}>Role Oversight</span>
      </div>
      <div style={{ overflowX: 'auto' }}>
        <table>
          <thead>
          ...
          <tbody>
            {['Ananya Rao', 'Vikram Shah', 'Meera Das'].map((name, i) => (
              <tr key={name}>
                <td>
                  <strong>{name}</strong>
                </td>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div className="branch-admin-grid">
        {branches.map(b => (
          <article key={b.id} style={{ background: '#ffffff', border: '1px solid #eee4d2', borderRadius: 16, padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#dfa500' }}>
              <Store size={20} />
              <strong style={{ fontSize: 15, color: '#1c1917' }}>{b.name}</strong>
            </div>
            <p style={{ fontSize: 11, color: '#78716c', margin: 0 }}>Bengaluru Zone • {b.distance} away</p>
      ...
      {message && (
        <div style={{ padding: 10, background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#166534', borderRadius: 10, fontSize: 11, fontWeight: 700 }}>
          <CheckCircle2 size={14} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 4 }} /> {message}
        </div>
      )}
    </div>
  )
