...
export function ProductsPage() {
  ...
  const save = event => {
    ...
    const changes = {
      ...
      adminImage: draft.image,
    }

    if (editingId) {
      updateProduct(editingId, changes)
    } else {
      addProduct({ ...changes, image: '' })
    }
    setShowForm(false)
  }
    ...
    <section className="admin-table-card">
      <div className="table-heading" style={{ flexWrap: 'wrap', gap: 10 }}>
        <div>
          ...
          <span style={{ fontSize: 11, color: '#78716c' }}>{products.length} products</span>
        </div>
      ...
      <div style={{ overflowX: 'auto' }}>
        <table>
          <thead><tr><th style={{ width: 64 }}>Image</th><th>Product</th><th>Category</th><th>Price</th><th>Details</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
      ...
      {showForm && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.52)', zIndex: 120, display: 'grid', placeItems: 'center', padding: 16 }} onClick={() => setShowForm(false)}>
        <form onSubmit={save} onClick={event => event.stopPropagation()} style={{ width: 'min(720px,100%)', maxHeight: '90vh', overflow: 'auto', background: '#fffdf9', borderRadius: 18, padding: 22, boxShadow: '0 24px 80px rgba(0,0,0,.18)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div><h2 style={{ margin: 0 }}>{editingId ? 'Edit Product' : 'Add Product'}</h2><p style={{ margin: '4px 0 0', fontSize: 12, color: '#78716c' }}>Upload an image for the admin catalog only.</p></div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.5fr) minmax(220px,.8fr)', gap: 16 }}>
            ...
            <div>
              <span style={{ fontSize: 11, fontWeight: 800 }}>Product image</span>
              <div style={{ marginTop: 8, border: '1px dashed #d9cdb9', borderRadius: 14, overflow: 'hidden', background: '#f7f2e9' }}>
                {draft.image ? <img src={draft.image} alt="Product preview" style={{ width: '100%', aspectRatio: '1', objectFit: 'cover', display: 'block' }} /> : <div style={{ aspectRatio: '1', display: 'grid', placeItems: 'center', color: '#8c806c' }}><div style={{ textAlign: 'center' }}><ImageIcon size={32} /><div style={{ fontSize: 11, marginTop: 6 }}>Choose a product image</div></div></div>}
                <div style={{ padding: 10 }}>
                  <button type="button" onClick={() => fileRef.current?.click()} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, width: '100%', padding: '9px 10px', borderRadius: 9, background: '#1c1208', color: '#fff', cursor: 'pointer', fontSize: 11, fontWeight: 800 }}><Upload size={13} /> Choose Image</button>
                  <input ref={fileRef} type="file" accept="image/*" onChange={chooseImage} style={{ display: 'none' }} />
                </div>
              </div>
              <p style={{ fontSize: 10, color: '#8c806c', lineHeight: 1.4, marginTop: 8 }}>This uploaded image is stored as <strong>adminImage</strong> and is intentionally not used by the customer panel.</p>
            </div>
          </div>
